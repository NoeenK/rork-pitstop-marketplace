import { View, Text, StyleSheet, TextInput, TouchableOpacity, Animated, Alert, Keyboard, TouchableWithoutFeedback, Platform, Image, Switch } from "react-native";
...
export default function SignUpEmailScreen() {
  ...
  const { signInWithGoogle, signInWithApple, pendingOAuthUser } = useAuth();
  ...
  const [isHandlingOAuth, setIsHandlingOAuth] = useState<boolean>(false);
  ...
  useEffect(() => {
    const handleOAuthUser = async () => {
      if (pendingOAuthUser && !isHandlingOAuth) {
        console.log('[SignUp] OAuth user detected:', pendingOAuthUser);
        console.log('[SignUp] Needs profile setup:', pendingOAuthUser.needsProfileSetup);
        setIsHandlingOAuth(true);
        
        console.log('[SignUp] Redirecting to welcome screen for profile setup');
        router.replace('/onboarding/welcome');
      }
    };

    handleOAuthUser();
  }, [pendingOAuthUser, isHandlingOAuth, router]);
