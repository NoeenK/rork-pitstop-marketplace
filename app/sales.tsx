...
export default function SalesScreen() {
  ...
  const userListings = mockListings.filter(l => l.sellerId === user?.id);
  const activeListings = userListings.filter(l => l.isActive && !l.isSold);
  const soldListings = userListings.filter(l => l.isSold);

  const displayListings = selectedTab === "active" ? activeListings : soldListings;

  const totalViews = userListings.reduce((sum, l) => sum + l.viewCount, 0);
  const totalSales = soldListings.length;
  const totalEarnings = soldListings.reduce((sum, l) => sum + (l.priceCents || 0), 0);
