...
import { mockChatThreads, mockMessages, getLastMessagePreview } from "@/mocks/chats";
  ...
  useEffect(() => {
    ...
    const loadThreads = async () => {
      ...
      if (error) {
        ...
        console.log("[ChatContext] Using mock threads");
        const threadsWithMessages = mockChatThreads.map(thread => ({
          ...thread,
          lastMessage: { text: getLastMessagePreview(thread.id) } as any,
        }));
      ...
      if (data && data.length > 0) {
        ...
        console.log("[ChatContext] No threads found, using mock data");
        const threadsWithMessages = mockChatThreads.map(thread => ({
          ...thread,
          lastMessage: { text: getLastMessagePreview(thread.id) } as any,
        }));
  ...
  const getMessagesByThreadId = useCallback(async (threadId: string) => {
    ...
    if (error) {
      ...
      if (mockMessages[threadId]) {
        console.log("[ChatContext] Using mock messages for thread:", threadId);
    ...
    if (mockMessages[threadId]) {
      console.log("[ChatContext] No database messages, using mock messages for thread:", threadId);
      return mockMessages[threadId];
    }
  ...
  const sendMessage = useCallback(async (threadId: string, text: string, senderId: string) => {
    try {
      ...
      if (error) {
        ...
        console.log("[ChatContext] Using mock mode - adding message locally");
        ...
        setMessages(prev => ({
          ...prev,
          [threadId]: [...(prev[threadId] || []), mockMessage],
        }));

        setThreads(prev =>
          prev.map(thread =>
            thread.id === threadId
              ? { ...thread, lastMessage: mockMessage, lastMessageAt: new Date() }
              : thread
          )
        );

        console.log("[ChatContext] Mock message added:", mockMessage.id);
        return mockMessage;
      }
